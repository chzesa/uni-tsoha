...
def create_thread(topic_url, title, url, content):
	...
	sql = "WITH ins1 AS (INSERT INTO posts(user_id, created)VALUES (:user_id, NOW()) RETURNING id), ins2 AS (INSERT INTO content(content, edited, edited_by, post_id) VALUES (:content, NOW(), :user_id, (SELECT id FROM ins1)) RETURNING post_id) INSERT INTO threads(topic_id, title, link, message_id) VALUES ((SELECT id FROM topics WHERE url = :topic_url), :title, :url, (SELECT id FROM ins1)) RETURNING id"
...
def create_topic(url, title, description):
	if not is_valid_topic_url(url) or not is_valid_topic_title(title):
		return False
	sql = "INSERT INTO topics(url, title, description, created, owner_id) VALUES (:url, :title, :description, NOW(), :id);"
...
def get_threads(topic):
	'''
	SELECT * FROM threads
		RIGHT JOIN (SELECT * FROM topics WHERE topics.url=:topic) AS T1 ON threads.topic_id = T1.id
	'''

	sql = "SELECT * FROM threads RIGHT JOIN (SELECT id FROM topics WHERE topics.url=:topic) AS T1 ON threads.topic_id = T1.id"
